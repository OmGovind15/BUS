...
class EncoderDecoder(BaseSegmentor):
    ...
    def generate_pseudo_label(self, img, img_metas, dilation_mask=None):
        self.update_debug_state()
        if self.debug:
            self.debug_output = {
                'Image': img,
            }
        out = self.encode_decode(img, img_metas,dilation_mask)
        if self.debug:
    ...
    def encode_decode(self, img, img_metas,dilation_mask, upscale_pred=True):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        x = self.extract_feat(img)
        out = self._decode_head_forward_test(x, img_metas,dilation_mask)
        if upscale_pred:
            out = resize(
                input=out,
                size=img.shape[2:],
    ...
    def _decode_head_forward_test(self, x, img_metas, dilation_mask):
        """Run forward function and calculate loss for decode head in
        inference."""
        seg_logits = self.decode_head.forward_test(x, img_metas, dilation_mask, self.test_cfg)
        return seg_logits
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        if batched_slide:
            ...
            crop_seg_logits = self.encode_decode(crop_imgs, img_meta, dilation_mask=None)
    ...
    def whole_inference(self, img, img_meta, rescale):
        ...
        seg_logit = self.encode_decode(img, img_meta,dilation_mask=None)
        if rescale:

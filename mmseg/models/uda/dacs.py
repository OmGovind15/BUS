...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        # modified
        self.DECON = cfg['DECON']
        self.kernel_size=cfg['kernel_size']
        self.crop_size=cfg['crop_size']
        self.AP = cfg['AP']
        self.SAM_Refinement = cfg['SAM_Refinement']
        self.RO = cfg['RO']
        self.SAM_ratio = cfg['SAM_ratio']
        self.pseudo_thres=cfg['pseudo_thres']
        self.resize_scale=cfg['resize_scale']        
    ...
    def get_pseudo_label_and_weight(self, logits, sam_masks_batch):
            ...
            if (self.max_iters/4)*(1-self.SAM_ratio) <=self.local_iter%(self.max_iters/4) <= (self.max_iters/4):
                ...
                for bb in range(len(sam_masks_batch)):
                    for mask in sam_masks_batch[bb]:
                        ...
                        pixel_per_class = pixel_per_class[:self.num_classes]# 0~13
        ...
        pseudo_weight_2 = torch.sum(ps_large_p).item() / ps_size
        pseudo_weight = torch.sum(ps_large_p).item() / ps_size
        pseudo_weight = pseudo_weight * torch.ones(
            pseudo_prob.shape, device=logits.device)
        return pseudo_label, pseudo_weight ,pseudo_weight_2
...
#######
            ...
            pseudo_label, pseudo_weight ,pseudo_weight_2= self.get_pseudo_label_and_weight(
            ...
            if self.RO:
                ...
                if scale_ in [0.25,0.5,0.75,1]:
                    img_hw=int(img.size()[-1])
                    x_=int(resize_img.size()[-1]/2)
                    y_=int(resize_img.size()[-1]/2)
                    if scale_==1:
                        x_choice=x_
                        y_choice=y_
                    else:
                        x_choice=np.random.choice(range(x_,img_hw-x_),1,replace=False)[0]
                        y_choice=np.random.choice(range(y_,img_hw-y_),1,replace=False)[0]

                    re_img=torch.zeros_like(img.long())
                    
                    re_img[:,:,x_choice-x_:x_choice+x_,y_choice-y_:y_choice+y_]=resize_img.long()
  
                    re_gt=torch.zeros_like(gt_semantic_seg)
                    re_gt[:,:,x_choice-x_:x_choice+x_,y_choice-y_:y_choice+y_]=resize_gt_semantic_seg.long()
                del resize_gt_semantic_seg,resize_img
...
#########
            ...
            #Making mask map for DECON loss
            if self.DECON:
                kernel = np.ones((self.kernel_size,self.kernel_size), np.uint8)
                private_mask_ori = torch.where(re_mixed_lbl==self.num_classes-1, 1, 0) # [2,1,1024,1024]
                ...
                dilation_mask = [private_masks, erosion_masks, dilation_masks,self.crop_size]
            else: 
                dilation_mask=None
            ...
            mix_loss.backward()
        del mix_loss,dilation_mask

        
        #Attaching Private
        if self.AP:
            if (self.max_iters/4)*(1-self.SAM_ratio) <=self.local_iter%(self.max_iters/4) <= (self.max_iters/4):
                source_mix_masks = (pseudo_label==self.num_classes-1)
                source_mix_weight=torch.where(source_mix_masks,pseudo_weight,gt_pixel_weight)
                source_mix_masks2=source_mix_masks.unsqueeze(1)
                m_pseudo_label=pseudo_label.unsqueeze(1)
                new_gt_semantic_seg=torch.where(source_mix_masks2,m_pseudo_label,gt_semantic_seg)
                new_source_img=torch.where(source_mix_masks2,target_img,img)
                
                mix_gt_pixel_weight=source_mix_weight
                mix_img=new_source_img
                mix_gt_semantic_seg=new_gt_semantic_seg
            else:
                mix_gt_pixel_weight=gt_pixel_weight
                mix_img=img
                mix_gt_semantic_seg=gt_semantic_seg
        else:
            mix_gt_pixel_weight=gt_pixel_weight
            mix_img=img
            mix_gt_semantic_seg=gt_semantic_seg
        ...
        clean_losses = self.get_model().forward_train(
            mix_img, img_metas, mix_gt_semantic_seg,seg_weight=mix_gt_pixel_weight,return_feat=True)
        ...
        if self.local_iter % self.debug_img_interval == 0 and \
            ...
            vis_re_mixed_img = torch.clamp(denorm(re_mixed_img, means, stds), 0, 1)
            for j in range(batch_size):
                ...
                if self.debug_gt_rescale is not None:
                    subplotimg(
                        ...
                        'Scaled GT',
                        cmap='cityscapes')
                subplotimg(axs[0][5], vis_re_mixed_img[j], 're_Mixed Image')
                if self.RO:
                    subplotimg(
                        axs[1][5], resize_class_masks[j][0], 're_size_Domain Mask', cmap='gray')
                    subplotimg(
                    axs[0][6],
                    re_mixed_lbl[j],
                    're mixed Seg (Pseudo)',

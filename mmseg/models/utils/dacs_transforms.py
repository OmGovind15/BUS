...
def get_class_masks_resize(labels):
    class_masks = []
    resize_class_masks=[]
    for label in labels:
        ...
        resize_class=[]
        classes22 = torch.unique(label)
        classes = classes[torch.Tensor(class_choice).long()]
        for i in [5,9,10,11,12]:
            if i in classes22 and i not in class_choice:
                resize_class.append(i)
        
        if len(resize_class)!=0:
            c=resize_class[np.random.choice(len(resize_class),1,replace=False)[0]]
        else:
            c=1000

        resize_class_masks.append(c)

        class_masks.append(generate_class_mask(label, classes).unsqueeze(0))
        
    return class_masks,resize_class_masks

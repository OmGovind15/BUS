...
#######
        ...
        if dilation_mask is not None:
            ...
            private_mask, erosion_mask, dilation_mask,crop_size = dilation_mask
            ...
            pri_mask=torch.logical_and((1-erosion_mask), erosion_mask)


            loss_total_dilation=0
            w_cnt=0
            cnt=0
            while w_cnt<1:
                w_cnt+=1
                ...
                #while is_private:
                feat_hw=fused_feat.shape[-1]
                p=crop_size
                h, w = torch.randint(p,feat_hw-p,(1,)), torch.randint(p,feat_hw-p,(1,))
                ...
                common_mask2 = common_mask[:,:,h-p:h+p,w-p:w+p].reshape(-1, 1).repeat(1,D)
                fused_feat2 = fused_feat[:,:,h-p:h+p,w-p:w+p].reshape(-1, D) 
                ...
                if torch.count_nonzero(private_mask2)>0 and torch.count_nonzero(erosion_mask2)>0 and torch.count_nonzero(dilation_mask2)>0 and torch.count_nonzero(common_mask2)>0:
                    cnt+=1
                    private_feat = fused_feat2[erosion_mask2!=0].reshape(-1, D)
                    ...
                    loss_total_dilation+=loss_dilation
            if cnt>0:
                loss_total_dilation=loss_total_dilation/cnt
                loss.update(add_prefix({'loss_dilation':loss_total_dilation}, 'dilation'))
